...
from app.services.ai.agent.summary_agent import SummaryAgentService

router = APIRouter(prefix="/agent", tags=["AI 学习助手 -- agent 流程"])

# 实例化 Agent 核心类
HomeworkAgent = HomeworkAgentService()
SummaryAgent = SummaryAgentService()
...
"""
    【学生端】Agent 对话总结接口
    接收前端传来的 question，返回总结结果并保存为 Markdown 文件。
"""
@router.post("/summary", response_model=AgentChatResponse, status_code=status.HTTP_200_OK)
async def agent_summary(payload: AgentChatRequest):
